...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"ets_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f ets-*.tar.gz
	echo Create archive \"ets-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../ets-$(VERSION)-$(BRANCH).tar.gz ets
...
ets.tar.gz: build
	cd make_build; tar czf ../ets.tar.gz ets && rm -rf ../make_build
